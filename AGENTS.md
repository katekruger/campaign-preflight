...
Findings only — none of these have been fixed. Verified at HEAD (`4dc2ab2`,
2026-09-01) against `README.md`, `CONTRIBUTING.md`, and this file's own text,
checked from outside the module each claim describes (its test suite, its CI
config, its actual runtime behavior) rather than from the function making the
claim.

- **This file's own "known coverage gap" is stale — the gap it describes is
  already closed.** The Testing conventions section above (and Non-negotiables)
  still says `_assert_read_only()`'s three `raise PreflightError(...)` branches
  have "zero coverage from the test suite" and calls it "a known coverage gap,
  not yet closed." That was true before PR #8 (`ddb88d9`, "test: cover
  `_assert_read_only()`'s fail-closed failure branches"), which is already
  merged into this branch. `tests/unit/test_mcp_safety.py` now exercises all
  three branches directly — `pytest.raises(PreflightError, match="mutating
  name")`, `match="does not declare itself READ-ONLY"`, and `match="not
  annotated readOnlyHint"` all appear in that file today. This is the single
  most safety-critical passage in the document, and it currently undersells
  work that has already landed — a future reader could reasonably (and
  wrongly) treat closing this gap as still-open work.
- **This file's own test count is stale.** Both the Commands section and
  Testing conventions say "1,851 tests." `uv run pytest --collect-only -q`
  currently sums to 1,896 — consistent with the 89 lines PR #8 added to
  `test_mcp_safety.py` landing after that count was last written.
- **README's architecture section names the wrong data-modeling tool.** "The
  context is a frozen Pydantic model, so 'a rule never mutates its input' is
  enforced by the type system rather than by review" (Architecture, near the
  mermaid diagram). `src/campaign_preflight/models.py`'s own module docstring
  says the opposite: "Implemented with frozen dataclasses and no third-party
  dependencies." `grep -n '^import\|^from' src/campaign_preflight/models.py`
  shows only `dataclasses`, `json`, `datetime`, `enum`, `typing` — no
  Pydantic import anywhere in the package. If this codebase actually depended
  on Pydantic, `security.yml`'s AST walk (every `src/` import must resolve to
  the standard library, or to the one allowed `httpx` extra) and
  `release.yml`'s bare-3.9-venv `--no-deps` install would both fail on every
  push — they don't, because the claim is wrong, not the guard. The freezing
  guarantee itself is real; only the tool named for it is wrong.
- **README's PyPI install instruction currently fails.** "### As a CLI ...
  `pipx install campaign-preflight`" is presented as a working install path
  alongside the plugin and from-checkout options. `.github/workflows/release.yml`
  has never actually published to PyPI: the `pypi` job carries `if: false`
  with the comment "PyPI publication is deliberately NOT automatic... Until
  then a release produces GitHub artifacts only." Running the documented
  command against a project that has never published a release produces "No
  matching distribution found," not an install. `pipx install
  campaign-preflight` should either be removed until the trusted-publisher
  environment exists, or marked not-yet-available.
- **An absolute claim with no test that would fail if it stopped being
  true.** "It does not verify mailboxes. Address checks are syntax only. No
  DNS, no SMTP." (README, "What it does not do"). `grep -rn "dns\|smtp\|DNS\|
  SMTP" tests/ src/` returns nothing — there is no test asserting that a
  contacts rule never performs a DNS lookup or SMTP handshake. Every other
  absolute claim swept this way had a direct counterpart (`test_unknown_
  deducts_nothing`, `test_blocker_always_produces_not_ready`, `test_no_
  spam_word_folklore_is_implemented`, `test_auth_error_never_echoes_the_key`,
  the full `test_instantly_transport.py` write-barrier matrix); this one
  currently has none. Nothing indicates the claim is false today — it is a
  divergence in waiting, not a bug, and belongs here rather than being
  silently fixed by adding a test as part of this pass.

## Module reachability

Report only — nothing below was fixed. Instrumented with `coverage run
--parallel-mode --branch --source=campaign_preflight`, driving only real
entry points, never the test suite: every CLI subcommand (`demo` in three
output formats, `check` against all three bundled examples, `rules list`,
`rules list --category ... --json`, `rules explain`, `validate-config`,
`version`, `instantly` with a dummy key so the provider actually constructs
and the transport actually runs before the network call fails), plus the
full MCP tool surface over real stdio JSON-RPC (`initialize`,
`notifications/initialized`, `tools/list`, `ping`, and all six registered
tools: `preflight_demo`, `preflight_files`, `preflight_instantly_campaign`,
`list_preflight_rules`, `explain_preflight_rule`,
`validate_preflight_config`). Coverage data from all runs combined.

**Result: every module under `src/campaign_preflight/` is both imported and
executes at least one function body when the real entry points are driven —
none are dead.** `diff` between `find src -name '*.py'` and the files
`coverage report` lists is empty; `coverage --source=campaign_preflight`
lists every file under that package whether or not it was ever imported, so
an unreached module would show up as a 0%-covered row rather than being
silently absent, and none is.

| Module | Imported | Called | Via |
|---|---|---|---|
| `__init__.py` | yes | yes | both entry points (package init) |
| `_yaml.py` | yes | yes | CLI (`load_config`, campaign/leads file parsing), MCP `preflight_files`/`validate_preflight_config` |
| `cli.py` | yes | yes | CLI, all subcommands |
| `config.py` | yes | yes | CLI (`demo`/`check`/`instantly`/`validate-config`), MCP `validate_preflight_config` |
| `engine.py` | yes | yes | CLI `demo`/`check`/`instantly`, MCP `preflight_demo`/`preflight_files`/`preflight_instantly_campaign` |
| `errors.py` | yes | yes | both entry points, every error and redaction path |
| `mcp/__init__.py` | yes | yes | MCP server startup |
| `mcp/formatting.py` | yes | yes | MCP, shaping every tool response |
| `mcp/protocol.py` | yes | yes | MCP, the JSON-RPC framing for every request in this run |
| `mcp/server.py` | yes | yes | MCP, tool registry + `_assert_read_only()` at startup |
| `models.py` | yes | yes | both entry points, every report |
| `normalization.py` | yes | yes | CLI `check`/`demo`, MCP `preflight_files`/`preflight_demo` |
| `providers/__init__.py` | yes | yes | both entry points |
| `providers/base.py` | yes | yes | both entry points, `ProviderResult` on every provider read |
| `providers/csv_provider.py` | yes | yes | CLI `check`, MCP `preflight_files` |
| `providers/fixture_provider.py` | yes | yes | CLI `demo`, MCP `preflight_demo` |
| `providers/instantly_provider.py` | yes | yes | CLI `instantly`, MCP `preflight_instantly_campaign` (fails on the network call with a dummy key, but constructs the provider and reaches the transport first) |
| `providers/instantly_transport.py` | yes | yes | same as above — `ReadOnlyTransport` runs before the network attempt |
| `reporting/__init__.py` | yes | yes | CLI rendering |
| `reporting/csv_export.py` | yes | yes | CLI `check --affected-csv` |
| `reporting/json_report.py` | yes | yes | CLI `--format json`, MCP tool responses |
| `reporting/markdown.py` | yes | yes | CLI `--format markdown` |
| `reporting/redaction.py` | yes | yes | both entry points, every report and error string |
| `reporting/terminal.py` | yes | yes | CLI default output |
| `rules/__init__.py` | yes | yes | both entry points, `all_rules()`/`get_rule()` |
| `rules/base.py` | yes | yes | `engine.evaluate()` on every run |
| `rules/campaign.py` | yes | yes | every `check`/`demo`/`instantly`/`preflight_*` run |
| `rules/contacts.py` | yes | yes | same |
| `rules/copy.py` | yes | yes | same |
| `rules/personalization.py` | yes | yes | same |
| `rules/schedule.py` | yes | yes | same |
| `rules/senders.py` | yes | yes | same |
| `rules/suppression.py` | yes | yes | same |
| `scoring.py` | yes | yes | `engine.evaluate()` on every run |

Statement coverage from this drive alone (not the test suite) ranged from
100% (`errors.py`, `scoring.py`, several `__init__.py` files) down to 37%
(`providers/instantly_provider.py` — expected, since the one `instantly` run
here fails at the network call and never exercises pagination, retries, or
the campaign cache). Low coverage on a module is not the same finding as an
unreached module: everything on this list ran real code, some of it just
didn't run *every* branch with the handful of invocations in this pass. No
module was excluded or sampled to produce this table.

### Process-restart check

No test in this repository restarts the process between assertions, and
none needs to: `grep -rn "open(\|\.write(\|tempfile\|pickle\|shelve\|sqlite"
src/campaign_preflight` turns up exactly three writers — the CLI's
`--output` report file, `reporting/csv_export.py`'s `--affected-csv`, and
`reporting/json_report.py`'s one-time read of the bundled JSON Schema file —
plus one in-memory-only cache, `InstantlyProvider._campaign_cache` (a plain
`dict` assigned in `__init__`), which is scoped to one provider instance
within one process and is never written to disk. There is no local
database, fixture cache, or run-state file that a later invocation reads
back, and therefore no live-path-versus-rebuild-from-storage pair to
disagree with each other the way the sibling repo's mailbox-pause state did.
Every invocation of this tool — CLI or MCP — reads directly from its source
(CSV files, YAML, or the live Instantly API) with nothing persisted in
between. If a caching layer is ever added ahead of the `instantly` provider,
this is the section to update, and it would be the first thing in this
repository actually worth a process-restart test.
